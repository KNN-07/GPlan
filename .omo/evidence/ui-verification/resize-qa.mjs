...
try {
  ...
  await page.screenshot({ path: `.omo/evidence/ui-verification/ui-resize-${phase}-desktop.png` });
  ...
  await page.screenshot({ path: `.omo/evidence/ui-verification/ui-resize-${phase}-mobile.png` });
  ...
  await page.screenshot({ path: `.omo/evidence/ui-verification/ui-resize-${phase}-side.png` });
  expect(await canvas.evaluate(element => element === document.querySelector(".exercise-viewer canvas"))).toBe(true);
  await page.getByRole("button", { name: "Close dialog", exact: true }).click();
  for (const width of [375, 360]) {
    await page.setViewportSize({ width, height: 900 });
    await page.getByRole("button", { name: "Exercises", exact: true }).click();
    await page.screenshot({ path: `.omo/evidence/ui-verification/ui-resize-${phase}-catalog-${width}.png` });
  ...
  await writeFile(`.omo/evidence/ui-verification/ui-resize-${phase}.json`, JSON.stringify(receipts, null, 2));
}

...
try {
  for (const [name, width, height] of [
    ...
    const shot = async (label) => {
      const path = `.omo/evidence/ui-verification/ui-${name}-${label}.png`;
  ...
  await browser.close();
  await writeFile(".omo/evidence/ui-verification/ui-browser-receipts.json", JSON.stringify({ receipts, browserClosed: true }, null, 2));
}

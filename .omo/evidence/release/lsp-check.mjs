...
const files = ['playwright.config.ts', 'tests/e2e/support.js', 'tests/e2e/release.spec.js', 'tests/e2e/providers.spec.js'];
...
child.stdout.on('data', chunk => {
  ...
  for (;;) {
    ...
    if (message.method === 'textDocument/publishDiagnostics' && files.some(file => message.params.uri.endsWith(file))) {
...
send('initialized', {});
for (const file of files) send('textDocument/didOpen', { textDocument: { uri: pathToFileURL(`${directory}/${file}`).href, languageId: file.endsWith('.js') ? 'javascript' : 'typescript', version: 1, text: await readFile(`${directory}/${file}`, 'utf8') } });
...
for (const [uri, values] of diagnostics) console.log(JSON.stringify({ file: uri.replace(pathToFileURL(directory).href + '/', ''), diagnostics: values }));
